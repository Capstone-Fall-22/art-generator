...
def get_navigation_links(initial_link: str, link_text: str, link_prefix: str='') -> list:
    '''
...
def get_image_links(navigation_links: list, image_link_class: str, link_prefix: str=''):
    image_links = list()
...
def download_images(image_links: list, output_dir: str):
    acceptible_licenses = [re.compile(f'Attribution-Share Alike {i}') for i in [2.0, 2.5, 3.0, 4.0]]
    ...
    with open(os.path.join(output_dir, 'attributions.json'), 'w') as attributions_file:
        for i, link in enumerate(image_links):
            ...
            try:
                download_link = soup.find('div', class_='fullImageLink').find('a')['href']
            except:
                continue
...
def aggregate_attributions(output_dir):
    ...
    for category in os.listdir(output_dir):
        ...
        with open(os.path.join(output_dir, category, 'attributions.json'), 'r') as attributions_file:
            ...
            if os.stat(os.path.join(output_dir, category, 'attributions.json')).st_size == 0: 
                continue
...
def scrape(category_urls, output_dir, max_categories=0):
    ...
    for i, category_url in enumerate(category_urls):
        navigation_links = get_navigation_links(category_url, 'next page', WIKIMEDIA_URL)
        image_links = get_image_links(navigation_links, 'galleryfilename', WIKIMEDIA_URL)
...
def main(*args, **kwargs):
    ...
    subcategory_depth = input("How many levels of subcategories to scrape? (Recommended: 2)\n> ")
    if not subcategory_depth.isdigit():
        sys.exit('Please try again with an integer')
    subcategory_depth = int(subcategory_depth)

    max_categories = input("What is the maximum number of categories to scrape (put 0 for no limit)\n> ")
